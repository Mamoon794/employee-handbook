...
export default function Dashboard() {
  ...
  useEffect(() => {
    const checkSubscriptionStatus = async () => {
      ...
      try {
        ...
        setShowPaywall(!response.data.subscribed);
      } catch (error) {
  ...
  const fetchCompanyDocs = useCallback(async () => {
    ...
    try {
      ...
      const get_files: pdfFile[] = (companyDocs.data?.companyDocs || []).map((doc: any) => ({
        name: doc.fileName,
  ...
  async function uploadDocuments(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      fetchCompanyDocs(); // Refresh the file list
    } catch (error) {
...
// import axiosInstance from '../axios_config';
// import PaywallModal from '../../../components/paywall-popup';
// >>>>>>> 0b4a225a42d97df434dd52ded76448fcae777a45
