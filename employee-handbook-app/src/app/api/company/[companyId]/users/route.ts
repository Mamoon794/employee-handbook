...
const VALID_SORTS = ["createdAt", "email", "firstName", "lastName", "province", "updatedAt", "userType"];
...
export async function GET(request: Request, { params }: { params: Promise<{ companyId: string }> }) {
    ...
    if (!VALID_SORTS.includes(sort)) {
        return new Response(JSON.stringify({ error: "Invalid sort parameter" }), { status: 400 });
    }
