...
export async function GET(request: Request) {
  try {
    ...
    const timeData: { [key: string]: { employees: number; questions: number; documents: number } } = {};
    ...
    if (showDailyData) {
      ...
      while (current <= endTimestamp) {
        ...
        const displayKey = current.toLocaleString('default', { weekday: 'short' });
      ...
      const current = new Date(startTimestamp.getFullYear(), startTimestamp.getMonth(), 1);
      const endMonth = new Date(endTimestamp.getFullYear(), endTimestamp.getMonth(), 1);
    ...
    employeeSnapshot.docs.forEach((doc: DocumentData) => {
      ...
      if (createdAt >= startTimestamp && createdAt <= endTimestamp) {
        const timeKey = showDailyData 
          ...
          : createdAt.toLocaleString('default', { month: 'short', year: 'numeric' });
        if (timeData[timeKey]) {
    ...
    if (allEmployeeIds.length > 0) {
      ...
      const chatsSnapshot = await chatsRef.where("userId", "in", allEmployeeIds).get();
      
      chatsSnapshot.docs.forEach((chatDoc: QueryDocumentSnapshot<DocumentData>) => {
    ...
    const generateDummyData = (timeLabel: string) => {
      const seed = timeLabel.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
    ...
    const result = timeLabels.map((timeLabel) => {
      const realData = timeData[timeLabel] || { employees: 0, questions: 0, documents: 0 };
      const dummyData = generateDummyData(timeLabel);
      
      return {
        month: keyToDisplay[timeLabel] || timeLabel,
        employees: realData.employees + dummyData.employees,
        questions: realData.questions + dummyData.questions,
        documents: realData.documents + dummyData.documents
      };
    });
