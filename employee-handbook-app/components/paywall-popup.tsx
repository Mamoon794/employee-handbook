    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-2 sm:p-4">
      <div className="bg-white rounded-2xl shadow-lg px-4 sm:px-6 lg:px-8 py-4 sm:py-6 max-w-sm sm:max-w-md w-full text-center relative max-h-[90vh] overflow-y-auto">
        ...
        <div className="mb-4 sm:mb-6">
          <h3 className="text-sm sm:text-base font-bold text-gray-800 mb-2 sm:mb-3 text-center">Premium Features:</h3>
        ...
        <div className="mb-4 sm:mb-6 p-2 sm:p-3 bg-[#f5f7fb] rounded-lg">
          <h3 className="text-sm sm:text-base font-bold text-gray-800 mb-2 sm:mb-3 text-center">Monthly Pricing:</h3>
          
          {/* Employee Count Display */}
          <div className="mb-3 sm:mb-4">
            <div className="flex flex-col sm:flex-row items-center justify-center space-y-1 sm:space-y-0 sm:space-x-2">
              <span className="text-xs sm:text-sm font-medium text-gray-700">Your Number of Employees:</span>
              {isLoadingCount ? (
                <span className="text-base sm:text-lg font-bold text-blue-800">Loading...</span>
              ) : (
                <span className="text-base sm:text-lg font-bold text-blue-800">{employeeCount}</span>
              )}
          ...
          {/* Pricing Tiers List */}
          <div className="mb-3 sm:mb-4">
            <h4 className="text-xs sm:text-sm font-bold text-gray-700 mb-2 sm:mb-3 text-center">Pricing Tiers:</h4>
            <div className="space-y-1 sm:space-y-2 max-h-32 sm:max-h-48 overflow-y-auto">
              {pricingTiers.map((tier, index) => {
                ...
                return (
                  <div key={index} className={`flex flex-col sm:flex-row justify-between items-start sm:items-center p-2 rounded-lg shadow-sm text-xs sm:text-sm ${
                    isCurrentTier ? 'bg-blue-100 border-2 border-blue-300' : 'bg-white'
                  }`}>
                    <span className={`${isCurrentTier ? 'font-bold text-blue-800' : 'text-gray-700'} mb-1 sm:mb-0`}>
        ...
        {error && <p className="text-red-500 mt-3 sm:mt-4 text-center text-xs sm:text-sm">{error}</p>}
      </div>
    </div>
  );
} 